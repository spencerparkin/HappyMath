...
namespace HappyMath
{
	...
	public:
		 ...
		 */
		void ToStandalonePolygonArray(std::vector<HappyMath::Polygon>& standalonePolygonArray) const;

		/**
		 * Clear this mesh and then add to it all polygons from the given array.
		 */
		void FromStandalonePolygonArray(const std::vector<HappyMath::Polygon>& standalonePolygonArray, double epsilon = 1e-6);
		...
		public:
			 ...
			 */
			void ToStandalonePolygon(HappyMath::Polygon& polygon, const PolygonMesh* mesh) const;

			/**
			 * Set this polygon using the given stand-alone polygon and mesh.
			 */
			void FromStandalonePolygon(const HappyMath::Polygon& polygon, PolygonMesh* mesh, double epsilon = 1e-6);
